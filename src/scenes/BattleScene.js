...
export default class BattleScene extends Phaser.Scene {
  ...
  create() {
    this.cameras.main.setBounds(0, 0, 3, 3);
    this.cameras.main.zoomTo(2);
    this.emitter = EventDispatcher.getInstance();
    this.model = this.sys.game.globals.model;

    this.cameras.main.setBackgroundColor('rgba(0, 55, 0, 0.5)');
  ...
  generateCharacter() {
    const blueDragon = new Enemy(this, 50, 80, 'blueDragon', 1, 'bug1', 100, 20);
    this.add.existing(blueDragon);

    const orangeDragon = new Enemy(this, 50, 130, 'orangeDragon', 1, 'bug2', 100, 8);
    this.add.existing(orangeDragon);

    const warrior = new Player(this, 250, 80, 'player', 1, 'senior dev', 100, 20);
    this.add.existing(warrior);

    const mage = new Player(this, 250, 130, 'player', 4, 'junior dev', 80, 8);
